...
class DemeritoController extends Controller
{
    //
        ...
        return view('forms.demerito.general',[
    ...
    public function  DatosList(){
        //  dd('llego');
          ...
          $trabajador_id = session('trabajador_Id');//obtenber los datos del trabajador a consultar
            ...
            $desplazamiento = DB::table('demerito')->where('id_trabajador',intval($trabajador_id))->where('estado',1)->get(); 
    ...
    public function eliminarDatos(Request $request){ 
        // dd($request);
       $codigo= DB::table('demerito')->select('id_doc')->where('codigo',$request->id)->get();
        //    dd($codigo[0]->id_doc);
