...
class MeritoController extends Controller
{
    ...
    public function Index(){
        ...
        return view('forms.meritos.general',[
    ...
    public function  DatosList(){
        //  dd('llego');
          ...
          $trabajador_id = session('trabajador_Id');//obtenber los datos del trabajador a consultar
            ...
            $desplazamiento = DB::table('merito')->where('id_trabajador',intval($trabajador_id))->where('estado',1)->get(); 
    ...
    public function eliminarDatos(Request $request){ 
        // dd($request);
       $codigo= DB::table('merito')->select('id_doc')->where('codigo',$request->id)->get();
        //    dd($codigo[0]->id_doc);
