...
var app = new Vue({
		...
		created: function () { 
			this.listarRegimenLaboral();
		},
		methods: { 
	listarRegimenLaboral()
	{ 
				// var url = '/fileacion-DJ';
		var url = '/regimenLaboral/list';	
     ...
     console.log(response.data);
					   this.pintarTablaRegimenLaboral(response.data);
						// this.	tablaDocumentos=response.data;
				}else{
					  document.getElementById("tbodyRegimenLaboral").innerHTML = "";
					  $('#tablaRegimenLaboral').DataTable();
				}
			})
			.catch((err) =>
			{
	...
	agregarSituacionLaboral(){   
		var url = '/regimenLaboral/agregar';
  var datosArray=$('#myFormSituacionAcademica').serialize();
		axios.post(url,datosArray).then((response) =>
			{   
				if (response.data[0] == "error")
				{  
     (typeof response.data.nombreSituacionLaboral != "undefined") ? $('#error_nombreSituacionLaboral').text(response.data.nombreSituacionLaboral): null; 
     (typeof response.data.detalleSituacionLaboral != "undefined") ? $('#error_detalleSituacionLaboral').text(response.data.detalleSituacionLaboral): null;  
				...
				{
     $('#error_nombreSituacionLaboral').text("");
     $('#error_detalleSituacionLaboral').text("");
					$('#nombreSituacionLaboral').val("");
	...
	{  
			...
			var url = '/regimenLaboral/eliminar' ; 
 ...
 {
  var situacionLaboralCodigo = $('#nivelSituacionLaboral').val();  
			var url = '/regimenLaboral/desactivar' ; 
				axios.post(url,{id:situacionLaboralCodigo}).then((response) =>
				{ 
 ...
 {
   var situacionLaboralCodigo = $('#nivelSituacionLaboral').val();  
			var url = '/regimenLaboral/habilitar' ; 
				axios.post(url,{id:situacionLaboralCodigo}).then((response) =>
				{ 
 ...
 mostrarSituacionLaboral(id)
 {
  ...
  $('#codigoRegistro').val(id);  
			...
			var url = '/regimenLaboral/consultar' ; 
				axios.post(url,{codigo:id}).then((response) =>
				{  
     let registro = response.data[0];
     console.log('registro',registro);
      $('#nombreSituacionLaboralUpd').val(registro.nombre);
      $('#detalleSituacionLaboralUpd').val(registro.descripcion);  
 ...
 {
  		var url = '/regimenLaboral/actualizar';
  var datosArray=$('#myFormSituacionAcademicaUpd').serialize();
		axios.post(url,datosArray).then((response) =>
			{   
				if (response.data[0] == "error")
				{  
     (typeof response.data.nombreSituacionLaboralUpd != "undefined") ? $('#error_nombreSituacionLaboralUpd').text(response.data.nombreSituacionLaboralUpd): null; 
     (typeof response.data.detalleSituacionLaboralUpd != "undefined") ? $('#error_detalleSituacionLaboralUpd').text(response.data.detalleSituacionLaboralUpd): null;  
				}
				else
				{
     $('#error_nombreSituacionLaboralUpd').text("");
     $('#error_detalleSituacionLaboralUpd').text("");
					$('#nombreSituacionLaboralUpd').val("");
     $('#detalleSituacionLaboralUpd').val("");
	 ...
	 pintarTablaRegimenLaboral(array)
	{
               
 
		var table = $('#tablaRegimenLaboral').DataTable();
