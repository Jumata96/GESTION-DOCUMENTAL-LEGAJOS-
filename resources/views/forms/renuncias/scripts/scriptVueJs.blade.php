...
var app = new Vue({
   ...
   nombreServidor   : "",
	...
	{  
		...
		var url = '/Renuncias-doc/agregar' + '?' + $('#myFormDJ').serialize();
	...
	eliminarDocumentos()
	{
			...
			var url = '/Renuncias-eliminarDocumento/' + codigoDJ; 
			axios.get(url).then((response) =>
			{   
						this.ListarRegistrosTablaSeccion01();
					this.listarDJ();
  ...
  document.getElementById("tbodyDj").innerHTML = "";
	...
	{ 
		...
		var url = '/Renuncias/agregar' + '?' + $('#myFormModalSeccion01').serialize();
		axios.post(url, formData,
			{
				headers:
				{
     ...
     (typeof response.data.fechaFinContrato != "undefined") ? $('#error_fechaFinContrato').text(response.data.fechaFinContrato): null; 
   ...
   if (elemento.estado==1) {
    ...
    botonAccion=botonHabilitar; 
			...
			$("#tbodyTabla01").append( 
                ...
                '<td>' + elemento.situacionLaboral + '</td>'+
                '<td>' + elemento.fechaResolucion + '</td>'+
                '<td>' + elemento.fechaFinContrato + '</td>'+ 
